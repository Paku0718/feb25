    ...
    const handleConfirmOrder = async (id) => {
        try {
            ...
            if (response.ok) {
                setCheckouts(prev =>
                    prev.map((checkout) =>
                        checkout.id === id ? { ...checkout, confirmedByAdmin: true, status: 'Confirmed' } : checkout
                    )
                );
            } else {
    ...
    const handleRejectOrder = async (id) => {
        try {
            ...
            if (response.ok) {
                setCheckouts(prev =>
                    prev.map((checkout) =>
                        checkout.id === id ? { ...checkout, confirmedByAdmin: false, status: 'Cancelled' } : checkout
                    )
                );
            } else {
    ...
    return (
        <>
            <div className="container mx-auto mt-8">
                ...
                <ul>
                    {checkouts.map((checkout) => (
                        <li key={checkout.id} className="mb-4">
                            <div className="bg-white p-4 rounded shadow">
                                ...
                                {checkout.status === 'Confirmed' ? (
                                    ...
                                    <>
                                        <button
                                            onClick={() => handleConfirmOrder(checkout.id)}
                                            className="bg-teal-500 text-white px-4 py-2 rounded mt-2 mr-4"
                                        >
                                            Confirm Order
                                        </button>
                                        <button
                                            onClick={() => handleRejectOrder(checkout.id)}
            ...
            <div className='ml-[650px] mt-8 mb-8'>
                <Link to="/admin" className='bg-teal-500 text-white px-4 py-2 hover:bg-teal-600 rounded-full'>Go Back</Link>
            </div>
        </>
    );
};
